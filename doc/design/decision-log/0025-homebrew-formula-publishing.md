...
GitHub の macOS runner には Homebrew が PATH にあり、Ubuntu の x86_64 runner には `/home/linuxbrew` に入っている。Ubuntu の arm64 runner には入っていない場合があり、その target は install を確かめられない。
...
- PR でも Formula で install する（F）。`release-verify` は dist の `build-global-artifacts`（Formula を作る job）と並行して走り、その artifact を待てない。このため `release-verify` の `homebrew-formula` job が、plan job が保存した同じ dist と同じ入力（全 target の archive と checksum）から Formula を作り直して検査する。`homebrew` job が、build と同じ native runner で `brew style`、local tap からの install、`brew test`、配置（bin、doc、`pkgshare` の third-party 表記）と archive の同一性を確かめる。Homebrew の無い `aarch64-unknown-linux-gnu` の runner に限って、確かめられなかったことを step summary に記録して終える。
...
- 公開 tap からの `brew install` / `upgrade`、tap への実 push、`aarch64-unknown-linux-gnu` での Formula の install は、初回公開（#40）まで確かめられない。
...
- Ubuntu の arm64 runner に Homebrew が入った場合、または Homebrew を入れる手段を検証済みにできた場合。`aarch64-unknown-linux-gnu` の install を必須にする。
