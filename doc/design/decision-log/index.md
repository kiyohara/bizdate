...
| 0022 | decided | release workflow の構成 | dist 0.32.0 の設定は `dist-workspace.toml`、`dist = true` だけ `Cargo.toml`。公開を止める検査は host が結果を見る段階に置き、plan-jobs は使わない（CI の再実行は local-artifacts、tag と archive の検証は global-artifacts）。third-party 表記は各 build job で生成して依存と照合。PR も archive まで作って検証し、公開は tag push のみ。profile と toolchain は CI に揃え、action は SHA 固定。ローカルは `release-tools` service | [0022-release-workflow.md](0022-release-workflow.md) |
