...
| 0023 | decided | model の識別子の記載範囲と review metadata の Model | 禁止は PR title だけ（tool 名と同じ）。description、PR / review のコメント、commit message、code と文書の本文は制限せず裁量に任せる。review metadata の `Model` は実行環境で確認した識別子を書き、実行環境の既定の指示を理由に `unknown` にしない | [0023-model-identifier-scope.md](0023-model-identifier-scope.md) |
