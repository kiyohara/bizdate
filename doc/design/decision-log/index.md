...
| 0015 | decided | Dependabot 更新 | GitHub Actions を週次・グループ化・open PR 上限 1 本で更新。bot PR は Issue 起点の対象外、レビューは直列、merge は人間。末尾は tag のみ。release workflow の導入に伴う再判断で、Cargo の version updates と Dependabot alerts / security updates を採用（実装は #63、settings はユーザーが有効化）。`release.yml` も更新対象に残し、差分が出たら dist の `github-action-commits` を揃える | [0015-dependabot-updates.md](0015-dependabot-updates.md) |
