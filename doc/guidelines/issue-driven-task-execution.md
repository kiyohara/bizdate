...
3. `main` を最新化し、Issue 記載のブランチ名で作業ブランチを作る（`doc/guidelines/git-operation-guidelines.md`）。cloud session（Claude Code on the web）では session 開始時に platform が作ったブランチをそのまま作業ブランチとして使い、Issue 記載のブランチ名へは切り替えない（同ガイドラインの「cloud session」）。note のファイル名は escape 規則により `draft_claude-<slug>.md` のようになる。
...
5. Issue の「作業内容」を実施する。「スコープ外」の事項には手を付けない。
