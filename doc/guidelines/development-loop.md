...
- MSRV: Dependabot は更新先を選ぶときに `Cargo.toml` の `rust-version` を考慮しない。CI は MSRV と同じ toolchain で回るため、MSRV を超える crate が入ると cargo が `rust-version` の不足で失敗する。この更新 PR は merge せず、MSRV を上げる Issue を起こして `doc/guidelines/development-command-guidelines.md` の「MSRV を上げるとき」に従い別 PR で上げる。更新 PR に MSRV の変更を足さない。上げるまでの間に更新 PR が open PR 上限を塞ぐ場合は、その version を `.github/dependabot.yml` の `ignore` へ理由のコメント付きで足す PR を出し、MSRV を上げたら外す。`@dependabot ignore` のコメントは設定がリポジトリに残らないため使わない。
