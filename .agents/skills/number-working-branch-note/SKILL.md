...
- `## 次にやること` など未完タスクを列挙するセクションに、**本 skill の終了時点で完了しているタスク行**（「PR を作成し、note を採番する」「PR 作成後、note を採番する」「PR を作成する」など）があれば、置換候補を提示し、ユーザー合意を得てから書き換える。機械的に書き換えない。`draft_` を含まない書き方でも対象とする。書き換え形式は checkbox（`- [ ]`）なら `- [x]`、そうでなければ行末に `（完了）` を付ける。行は削除しない（note は作業ログであり、採番時点で何が完了していたかを残すため）。
...
- 本 skill の終了時点で完了しているタスク行が description にもあれば、Step 5 と同じく置換候補を提示し、ユーザー合意を得てから書き換える。対象外タスク、複合行、完了を判断できない行の扱いも Step 5 と同じとする。
- title は通常触らない。
