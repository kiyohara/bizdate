...
Step 5 と Step 10 で **表現として** 置換してよいのは、次の定型に限る。完了タスク行の書き換えはこの表ではなく、Step 5 の判定基準に従う。
...
各ステップで失敗・矛盾を検出したら停止し、ユーザーに報告する。安全側に倒し、判断に迷うときは進めずに確認する。ただし完了タスク行の判定に迷う場合は、確認のために止めず、Step 5 に従って触らないまま終了時に報告する。
...
- `## 次にやること` など未完タスクを列挙するセクションに、**本 skill の終了時点で完了しているタスク行**（「PR を作成し、note を採番する」「PR 作成後、note を採番する」「PR を作成する」など）があれば、ユーザー合意を待たずに書き換える。次の 2 つの bullet の判定基準で完了と判定できる行が対象であり、判定できない行は触らない。`draft_` を含まない書き方でも対象とする。書き換え形式は checkbox（`- [ ]`）なら `- [x]`、そうでなければ行末に `（完了）` を付ける。行は削除しない（note は作業ログであり、採番時点で何が完了していたかを残すため）。
- 完了したかどうかは、**本 skill の終了時点で未完のまま残るか** で判断する。review 対応、merge 待ち、後続 Issue への着手、`progress.md` の PR 番号反映などは残るため対象外とする。Step 7 が commit 対象を `working-branch-notes/` 配下に限定しているとおり、本 skill は `progress.md` を更新しない（索引表の更新は `doc/guidelines/issue-driven-task-execution.md` の step 7 で行う）。一方、PR の作成のように本 skill の前提条件として実行前に満たされている要素と、note の rename や採番のように本 skill の実行そのもので達成される要素は、未完のまま残らないため完了として扱う。その達成を確認したり記録したりする作業は、本 skill の外側が終了後に行うため、残るものとして対象外とする。
- 1 行に完了している要素と未完のまま残る要素が混在する行（「PR を作成し、採番後に note rename と `progress.md` の PR 番号反映を行う」など）は、未完了の部分が隠れるため行全体を完了扱いにしない。触らず、終了時に報告する。完了したかどうかを判断できない行も同じ扱いとする。
- 本文中のファイル名置換だけを行うと、「これから採番する」という文意の行が新名のまま残って自己矛盾する。置換した行が完了タスク行に該当しないかを確認する。
...
- 本 skill の終了時点で完了しているタスク行が description にもあれば、Step 5 と同じくユーザー合意を待たずに書き換える。判定基準、書き換え形式、対象外タスク、複合行、完了を判断できない行の扱いも Step 5 と同じとする。
- title は通常触らない。
...
- 完了として書き換えたタスク行の一覧（note 本文 / PR description）。合意を待たずに書き換えるため、この報告がユーザーの確認経路になる。
- 触らなかった stale 表現・タスク行があればその一覧（定型に当てはまらない / 完了要素と未完了要素が混在する / 完了を判断できない）。
...
- 定型に当てはまらない stale 表現、複合行、完了を判断できないタスク行の書き換え（いずれも触らず、終了時に報告する）。
- title の書き換え（ユーザーの明示指示がある場合を除く。Step 10 の「title は通常触らない」に対応する）。
- 機械的に判定できる完了タスク行の書き換えに、ユーザー合意を求めて処理を止めること。
