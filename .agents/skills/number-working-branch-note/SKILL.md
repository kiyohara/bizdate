...
- 本 skill の実行では完了しないタスク（review 対応、merge 待ち、後続 Issue への着手、`progress.md` の PR 番号反映など）は対象外とする。Step 7 が commit 対象を `working-branch-notes/` 配下に限定しているとおり、本 skill は `progress.md` を更新しない（索引表の更新は `doc/guidelines/issue-driven-task-execution.md` の step 7 で行う）。
...
- 定型に当てはまらず、または完了の判断が付かず触らなかった stale 表現・タスク行があればその一覧。
