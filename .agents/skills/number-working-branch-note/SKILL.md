...
- `## 次にやること` など未完タスクを列挙するセクションに、**本 skill の終了時点で完了しているタスク行**（「PR を作成し、note を採番する」「PR 作成後、note を採番する」「PR を作成する」など）があれば、ユーザー合意を待たずに書き換える。次の bullet の判定基準で完了と判定できる行が対象であり、判定できない行は触らない。`draft_` を含まない書き方でも対象とする。書き換え形式は checkbox（`- [ ]`）なら `- [x]`、そうでなければ行末に `（完了）` を付ける。行は削除しない（note は作業ログであり、採番時点で何が完了していたかを残すため）。
- 完了したかどうかは、**本 skill の終了時点で未完のまま残るか** で判断する。review 対応、merge 待ち、後続 Issue への着手、`progress.md` の PR 番号反映などは残るため対象外とする。Step 7 が commit 対象を `working-branch-notes/` 配下に限定しているとおり、本 skill は `progress.md` を更新しない（索引表の更新は `doc/guidelines/issue-driven-task-execution.md` の step 7 で行う）。一方、PR の作成のように本 skill の前提条件として実行前に満たされている要素は、未完のまま残らないため完了として扱う。
...
- 完了として書き換えたタスク行の一覧（note 本文 / PR description）。合意を待たずに書き換えるため、報告がユーザーが内容を確認する経路になる。
...
- 定型に当てはまらない stale 表現、複合行、完了を判断できないタスク行、title の書き換え（触らず、終了時に報告する）。
