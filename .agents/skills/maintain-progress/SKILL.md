...
- `progress.md` の「リリース履歴」はリリース台帳である。列と更新の責任は同節に、公開の手順は `doc/guidelines/release-guidelines.md` にある。
- 行を足すのは公開担当者であり、公開後確認 Issue を入力とする PR で足す。本 skill では行を足さず、見出しと列構成を変えない。
- 台帳の行は履歴として残し、観点 3 の圧縮の対象にしない。
- 「未確認」欄が指す追跡 Issue が完了していれば、その欄を更新してよい。
...
- PR を出した場合、`number-working-branch-note` から引き上げた項目（完了として書き換えたタスク行の一覧と、触らなかった stale 表現・タスク行の一覧。途中で停止した場合は停止理由と未反映の変更。0 件または呼ばなかった場合はその旨）を報告している。
