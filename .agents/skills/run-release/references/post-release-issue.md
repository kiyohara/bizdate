...
- [ ] README のインストール案内を、予定表記から有効な案内へ切り替える（初回公開のみ）
...
- この Issue の状態が確認済みであること（公開後の PR を出す条件）

## 実行

公開と公開後確認は `run-release`、公開後の PR は `run-issue-task` で行う。
````
