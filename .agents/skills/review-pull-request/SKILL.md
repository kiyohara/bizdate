...
description: bizdate の Pull Request を対象に、PR review、review comment 対応、対応結果の再確認を github-op-integrated MCP-first の workflow として実行する。bizdate リポジトリで PR をレビューするとき、review comment へ対応・返信するとき、対応結果を再検証して resolve 可否を返信するときに使う。汎用の gh-address-comments 系 skill / plugin と同時に該当する場合も、本 skill と project guideline の tool routing を優先する。
argument-hint: "<review | address-comments | verify-comments> [PR番号 | PR URL]"
---

# review-pull-request

bizdate の PR review を、project guideline に沿った github-op-integrated MCP-first workflow として実行する skill。PR 自体の review、既存 review comment への対応、対応結果の再確認と resolve 可否の判定までを 3 モードで扱う。inline thread の resolve 自体は人間が GitHub UI で行う（「Review event と resolve の制約」を参照）。
...
- `doc/guidelines/working-branch-notes-handling.md` / `doc/guidelines/working-branch-notes-security.md`
...
- `Mode` は投稿を行ったモード名とする。
- `address-comments` の返信と `verify-comments` の完了要約では、`Review cycle` に対象 review cycle の ID（元 review の値）をそのまま使い、新しい ID を作らない。同じ cycle を追跡可能に保つためである。
...
| 対象 | 編集の手段 |
| --- | --- |
| PR conversation comment（完了要約など） | MCP の `update_issue_comment`（cloud session の組み込み tool）。ローカルの `github-op-integrated` は allowlist に無いため、`gh api -X PATCH repos/{owner}/{repo}/issues/comments/{id}` |
| inline comment とその返信 | `gh api -X PATCH repos/{owner}/{repo}/pulls/comments/{id}`。MCP tool に編集の手段は無い |
...
- コメントの prefix は `.github/copilot-instructions.md` の体系（`[must]` / `[ask]` / `[imo]` / `[nits]` / `[fyi]`）に揃える。人間と Copilot と AI agent のレビューで語彙を共通にするためである。

## 組み込み / 汎用 review capability の再利用

- 本 skill は、各 Agent の native / built-in review capability（例: Claude Code の `code-review` skill、Codex の review mode）や、利用可能な汎用 review skill / plugin を置き換えない。
- 各モードで、利用可能な review capability を diff 分析、指摘の分類、妥当性判断、修正内容の再検証に利用してよい。
- 本 skill が専有する責務は次のとおり: 対象 PR と head SHA の確定、bizdate の正本との照合、github-op-integrated MCP-first の tool routing、review cycle 管理、Agent 識別とコメント形式、コメント投稿、read-back、resolve 可否の判定、完了条件。
- 汎用 capability が GitHub app / `gh`-first の取得・投稿手順、独自の approve / request changes、独自の完了条件を持つ場合、その部分は採用せず、本 skill と project guideline で置き換える。GitHub への直接投稿 option は使わず、findings の生成までにとどめ、投稿は本 skill の github-op-integrated 経路へ一元化する。
- 課金や cloud 実行を伴う review 機能は、ユーザーの明示指示なしに起動しない。
- 汎用 capability が利用できない環境でも処理を停止せず、Agent 自身の review 能力で同じ完了条件を満たす。
- 汎用 capability の出力は補助的な分析結果として扱い、最終的な指摘、返信、resolve の妥当性は現在の Agent が bizdate の Issue・正本・実装に照らして確認する。
...
- 本 skill は inline thread の resolve を自動実行しない。MCP tool や `gh api graphql` の機能制約ではない。fine-grained PAT では `resolveReviewThread` が `Contents: Read and Write` を要求するが、本プロジェクトは `Contents: write` を付与しない方針であるため、resolve は人間が GitHub UI で行う。
...
- review の dismiss、PR の merge、reviewer request の変更は本 skill から自動実行しない。

## permission

- fine-grained PAT は repository access に bizdate を含め、Pull requests / Issues の read / write と、Actions / Checks の read を許可する。review 作成、inline reply、conversation comment、CI 確認はこの範囲で実行できる。
- thread resolve は Pull requests permission だけでは実行できず、`Contents: Read and Write` が必要である。本プロジェクトはこれを付与しないため、resolve は自動実行の対象外とする。
- `Contents: write`、`Actions: write`、merge、release、workflow dispatch、repository settings などの追加 permission は本 skill のために付与しない。
- 修正の commit / push は local git で行い、`doc/guidelines/git-operation-guidelines.md` に従う。
...
3. MCP 未反映で、かつ `doc/guidelines/github-mcp-guidelines.md` の fallback 条件を満たす場合だけ、`doc/guidelines/github-cli-guidelines.md` に従って `gh` へ fallback する。
