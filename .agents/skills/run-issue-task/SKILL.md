---
name: run-issue-task
description: bizdate の GitHub Issue 駆動タスクを開始・実行する。Issue 番号を入力として受け取り、Issue 本文の依存確認から、作業ブランチ作成、working branch note 作成、実装、検証、progress.md 更新、PR 作成までを `doc/guidelines/issue-driven-task-execution.md` に準じて進める。Issue 番号が無い場合は `progress.md` の進行中タスク索引から次に進める候補をサジェストする。
---
...
- `doc/guidelines/github-cli-guidelines.md` — MCP fallback としての `gh`。
- `doc/guidelines/git-operation-guidelines.md` — commit 署名、remote、保護ブランチ。
- `doc/guidelines/working-branch-notes-handling.md` / `doc/guidelines/working-branch-notes-security.md` — note の作成・rename・禁則。
...
1. GitHub MCP tool で対象 Issue を読む。
   - Issue 本文、state、labels、comments、関連 PR の有無を確認する。
   - MCP tool が使えない場合のみ、`doc/guidelines/github-mcp-guidelines.md` の fallback ルールに従う。
2. Issue の依存を確認する。
   - 依存が `progress.md` の索引表にあれば、その行の状態と PR 欄で確認する。
   - 索引表に無ければ、依存先 Issue / PR が完了または merge 済みかを MCP tool で確認する。
   - 未完了の依存があれば作業を始めず、未完了依存を報告して終了する。
3. `main` を最新化し、作業ブランチを作る。
   - Issue に推奨ブランチ名があればそれを使う。無ければ Issue title から短く意味が分かる kebab-case の名前を決める。prefix は付けない。
   - `main` は保護されている。直接 push しない。
   - cloud session（Claude Code on the web）では platform が作ったブランチをそのまま使い、Issue の推奨ブランチ名へは切り替えない（`doc/guidelines/git-operation-guidelines.md` の「cloud session」）。
4. `working-branch-notes/draft_<escaped-branch-name>.md` を作る。
   - `working-branch-notes/_template.md` を基本形にする。
   - note に秘密情報・個人情報を書かない。
5. Issue の「作業内容」を実施する。
   - 「スコープ外」に挙がる事項へ手を広げない。
6. Issue の「検証」をすべて実行し、結果を note の検証セクションに記録する。
   - 検証していないことを検証済みとして書かない。実行できなかった項目は理由とともに残す。
7. 対象 Issue が `progress.md` の進行中タスク索引にある場合は、該当行を更新する。
   - 状態は PR merge 後に正になる内容として `done` へ進めてよい。
   - PR 作成前は PR 欄を `-` のままにし、採番後に `#<PR 番号>` へ更新する。
   - 単発 Issue が索引に無い場合は、無理に `progress.md` へ登録しない。
8. PR を作る。
   - description は日本語で、`Closes #<Issue 番号>` を含める。
   - merge はしない。
9. PR 採番後、note を `<PR 番号>_<escaped-branch-name>.md` へ rename する。
...
本 skill が手順の中で別 skill を呼ぶ場合、その skill の報告は本 skill の報告の中で要約されるため、ユーザーへ届くかどうかは本 skill の報告項目で決まる。そこで次を守る。

- 被委譲 skill が、ユーザーの確認や判断の起点として位置づけている報告項目は、要約で落とさず本 skill の報告へそのまま含める。該当するかは本 skill 側で改めて判断せず、被委譲 skill の記述に従う。
- 該当が 0 件の場合も、0 件であることを報告する。項目ごと省くと、確認対象が無かったのか報告が落ちたのかを区別できないためである。
- それ以外の報告項目は、未解決事項として扱うべきものを除き、要約してよい。

現時点の該当項目は次のとおり。被委譲 skill の報告項目を変えるときは、この一覧も揃える。

| 被委譲 skill | 呼び出し元 | 引き上げる項目 |
| --- | --- | --- |
| `number-working-branch-note` | step 9 | 完了として書き換えたタスク行の一覧（note 本文 / PR description）。同 skill の「終了時の報告」が、合意を待たずに書き換える代わりのユーザーの確認経路と位置づけている |

`number-working-branch-note` が触らなかった stale 表現・タスク行の一覧は、ユーザーが手で直すかを判断する対象であるため、未解決事項として報告する。
