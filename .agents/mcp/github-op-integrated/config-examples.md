...
Claude Code は repo root の `.mcp.json` を project scope として読む。この相対 `command` は MCP host が project root を基準に解決する前提である。wrapper 起動後は wrapper 自身の配置から project root を解決する。
...
`.mcp.json` / `.codex/config.toml` と揃えて project root 基準の相対 `command` を使う。相対 path は IDE / agent の両方で project root を基準に解決される。cursor-agent(CLI / Agent chat)では `${workspaceFolder}` を利用できないため、相対 path で揃えている。
