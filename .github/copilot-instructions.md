...
- dist 0.32.0 が checksum file（`.sha256` と `sha256.sum`）の末尾に足す空行は、`release-verify.yml` が取り除く。`build-global-artifacts` と並行して走るため、この run の artifact の一覧を読んで待ち（`actions: read`）、build の workflow artifact を `overwrite` で置き換えてから、`host` と同じ形で検査する（decision log 0022 の 2026-09-25 追記）。待ち合わせと置き換えの仕組み自体は指摘しない。置き換えが、build の artifact を取得する `archive`、`homebrew-formula`、`homebrew` の job と並行しうる変更と、`checksum files` の検査を外す・緩める変更は指摘する。
